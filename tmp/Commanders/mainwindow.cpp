...
void MainWindow::on_PB_BLTSEND_clicked()
{
    ...
    if(ui->CB_BLTAPPEND->currentText().compare("无追加")==0);
    else if(ui->CB_BLTAPPEND->currentText().compare("\\n")==0) text=QString("%1\n").arg(text);
    else if(ui->CB_BLTAPPEND->currentText().compare("\\r")==0) text=QString("%1\r").arg(text);
    else if(ui->CB_BLTAPPEND->currentText().compare("\\n\\r")==0) text=QString("%1\n\r").arg(text);
    else if(ui->CB_BLTAPPEND->currentText().compare("\\r\\n")==0) text=QString("%1\r\n").arg(text);


    m_pKer->BLTSend(text);
    //m_pKer->BLTSend(QString("\r\n"));
}
...
void MainWindow::BLTRecv(QByteArray content){

    QString text;
    if(ui->CB_BLTTEXTMODE_RECV->currentText().compare("文本")==0){
        text=QString(content.toStdString().c_str());
    }else{
        text=QString(content.toHex().toStdString().c_str());
    }
